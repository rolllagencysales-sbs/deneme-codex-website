...
const services = [
  {
    ...
    description: "Markanız için tamamen size özel ve tercihlerinize uygun web siteleri tasarlıyoruz. Her detayda profesyonellik ve estetiği ön planda tutuyor, güçlü SEO optimizasyonu ile arama motorlarında görünürlüğünüzü artırıyoruz.",
    icon: (
  ...
  },
  {
    ...
    description: "Markanızın dijital ve fiziksel dünyadaki kimliğini tamamen size özel tasarlıyoruz. Logo, renk paleti, tipografi ve tüm görsel öğeler, markanızın karakterini güçlendirecek şekilde tasarlanır.",
    icon: (
  ...
  },
  {
    ...
    description: "Markanızın hikayesini ve değerlerini etkileyici bir şekilde anlatan tamamen size özel tanıtım animasyonları hazırlıyoruz. Görsel efektler, akıcı geçişler ve yaratıcı konseptlerle desteklenir.",
    icon: (
  ...
  },
  {
    ...
    description: "Markanızın sosyal medya hesapları için tamamen size özel içerikler tasarlıyoruz. Yaratıcı grafikler, dikkat çekici görseller ve etkileyici metinlerle etkileşiminizi artırıyoruz.",
    icon: (
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="reveal animate-fade-up stagger-2 font-body text-foreground/60 text-lg max-w-2xl mx-auto">
            Sosyal medya içerik üretimi, web tasarım, marka kimliği tasarımı ve benzeri dijital hizmetler alanlarında uzman ekibimizle yanınızdayız.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {services.map((service, i) => (
            ...
            >
              <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-6 text-primary-foreground transition-transform duration-500 group-hover:scale-110" style={{ background: "var(--gradient-accent)" }}>
                {service.icon}
              </div>
              <h3 className="font-heading text-xl font-bold mb-3 group-hover:text-primary transition-colors duration-300">
                {service.title}
              </h3>
              <p className="font-body text-foreground/55 leading-relaxed text-sm">
                {service.description}
              </p>
            </div>
          ))}
